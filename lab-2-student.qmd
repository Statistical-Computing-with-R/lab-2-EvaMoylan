...
The dimensions of these data are 30,486 rows and 15 columns.

**3. What are the data types of the variables in this dataset?**

*The variables and their datatypes are: 'record_id' (double), 'month' (double), 'day' (double), 'year' (double), 'plot_id' (double), 'sex' (character), 'hindfoot_length' (double), 'weight' (double), 'date' (date), day_of_week' (character), 'plot_type' (character), 'genus' (character), 'species' (character), 'taxa' (character).*
...
#Scatter plot to compare weight and hindfoot length
ggplot(data = surveys,
...
#Species plotted across y-axis
ggplot(data = surveys,
       mapping = aes(x = weight,
                     y = species)) +
  geom_boxplot(outliers = FALSE) + 
  geom_jitter(width = 0.5, color = "tomato3", alpha = 0.3) +
  theme_bw() + 
...
*Done! ggplot2 package description used*
...
#Run an anova comparing population mean weight between rodent species
...
*At least one rodent species has a different population mean weight (df = 13, F = 19916, P < 0.001).*
